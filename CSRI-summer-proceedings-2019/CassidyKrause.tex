...
Stiff initial value problems, like the ones found in HOMME-NH 
nonhydrostatic atmosphere model, require a very small step-size when 
integrated with an explicit method. On the other hand, using a fully 
implicit method requires nonlinear solves, which are computationally 
expensive. To deal with these restrictions, two popular approaches have 
...
method on $s(\CFKU)$, and an explicit RK method to solve $n(\CFKU)$. This 
approach is efficient and accurate, and for this reason, IMEX RK methods 
are currently implemented with the HEVI partitioning in HOMME-NH. That is, 
an explicit RK method is used for the horizontal part, and an implicit RK 
method is used on the vertical part.
...
HOMME-NH. With IFRK methods, we split our equation 
as \[ \CFKU_t = f(\CFKU) = L_m\CFKU +  N_m(\CFKU),\] where $L_m$ is a linear operator 
containing the stiff part of the equation, and $N_m(\CFKU) = f(\CFKU)-L_m\CFKU$ 
contains the nonstiff terms. Often, $L_m = f'(\CFKU)$, but we do not assume 
this is the case. Using the change of variables $\CFKV(t) = 
e^{-L_m(t-t_m)}\CFKU(t)$, we have
\begin{equation}\label{CFK:eqn:vode}
\CFKV_t = e^{-L_m(t-t_m)}N(e^{L_m(t-t_m)}\CFKV).
\end{equation} 
With this new formulation, we can now use a fully explicit $r-$stage RK 
method to solve equation \ref{CFK:eqn:vode}.
...
the implementation of IFRK methods with HOMME-NH; Section \ref{CFK:sec:ifrk} 
...
in practice, but advances in efficiently forming $e^A$ have 
